...
chrome.storage.local.get((items) => {
    ...
    let storedWhitelist = items.ultrablock_whitelist;
...
chrome.webRequest.onBeforeRequest.addListener((details) => {
    ...
    if (whitelistedSites.includes(urlDictionary.get(details.tabId))) {
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (changeInfo.status === 'loading') {
        ...
        if (whitelistedSites.includes((_b = tab.url) === null || _b === void 0 ? void 0 : _b.split('/')[2])) {
...
chrome.tabs.onActivated.addListener((activeInfo) => {
    ...
    let url = urlDictionary.get(activeInfo.tabId);
    /* In case if the url is present in the whitelist (i.e. user disabled sites)*/
    /* Then change the icon to show the inactivity status */
